...
var time_since_last_range_attack := Time.get_ticks_msec() #tempo do ultimo tiro
var time_since_prep_range_attack := Time.get_ticks_msec() #tempo para desviar do tiro(entre preparar e atacar
...
func handle_input() -> void:
	if player != null and can_move():
		if can_respawn_knives or has_knife or has_gun: #se segurar arma, vai para posição de atirar
			goto_range_position()
		else: #se não segurar arma, vai para posição de bater
			goto_melee_position()

func goto_range_position() -> void:
	var camera := get_viewport().get_camera_2d()
	var screen_width := get_viewport_rect().size.x
	var screen_left_edge := camera.global_position.x - screen_width / 2
	var screen_right_edge := camera.global_position.x + screen_width / 2
	
	var left_destination := Vector2(screen_left_edge + EDGE_SCREEN_BUFFER, player.global_position.y)
	var right_destination := Vector2(screen_right_edge - EDGE_SCREEN_BUFFER, player.global_position.y)
	var closest_destination := Vector2.ZERO
	if (left_destination - global_position).length() < (right_destination - global_position).length():
		closest_destination = left_destination
	else:
		closest_destination = right_destination
	if (closest_destination - global_position).length() < 1:
		velocity = Vector2.ZERO
	else:
		velocity = (closest_destination - global_position).normalized() * speed
	
	if can_range_attack() and has_knife and projectile_aim.is_colliding(): #atira faca
		state = State.THROW
...
func handle_prep_shoot() -> void:
	if state == State.PREP_SHOOT and (Time.get_ticks_msec() - time_since_last_range_attack > duration_prep_range_attack):
		shoot_gun()
		time_since_last_range_attack = Time.get_ticks_msec()
...
func goto_melee_position()-> void: #posição de bater
